  ...
  <div class="analytics-container">
    <div v-if="analyticsData">
      ...
      <div class="seasons-grid">
        <div
          class="season"
          v-for="season in analyticsData"
          :key="season.seasonOrder"
        >
          <h3>Season {{ season.seasonOrder }}</h3>
          <div class="analytics-details">
            <p>
              <strong>Total Public Addresses:</strong>
              {{ season.analytics.trackSession.totalPublicAddress }}
            </p>
            <p>
              <strong>Total Sessions:</strong>
              {{ season.analytics.trackSession.totalSessions }}
            </p>
            <h4>VIP Track</h4>
            <ul>
              <li v-for="vip in season.analytics.trackVip" :key="vip._id">
                <strong>{{ vip._id }}:</strong> <strong>PVU Amount:</strong>
                {{ vip.pvuAmount }}, <strong>Buy Count:</strong>
                {{ vip.buyCount }}
              </li>
            </ul>
            <p>
              <strong>Total Reach 1M:</strong>
              {{ season.analytics.reach1M.total }}
            </p>
            <p>
              <strong>Average Time in Seconds:</strong>
              {{ season.analytics.reach1M.avgTimeInSecond }}
            </p>
          </div>
        </div>
      </div>
